...
static servo_status_t servo_write_packet(uint8_t id, uint8_t addr, uint8_t *data, uint8_t len, uint8_t cmd)
{
    // Check length
...
static servo_status_t servo_read_word(uint8_t id, uint8_t addr, uint16_t *data)
{
    // Send out INST_READ
...
servo_status_t servo_set_position(uint16_t position, uint16_t time, uint16_t speed)
{
    ...
    return servo_write_packet(servo_id, SCS0009_GOAL_POSITION_L, buf, 6, INST_WRITE);
}
