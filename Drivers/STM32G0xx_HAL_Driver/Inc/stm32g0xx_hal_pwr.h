...
#define PWR_WAKEUP_PIN1_LOW                                                                                            \
    ((PWR_CR4_WP1 << PWR_WUP_POLARITY_SHIFT) | PWR_CR3_EWUP1) /*!< Wakeup pin 1 (with low level detection) */
#define PWR_WAKEUP_PIN2_LOW                                                                                            \
    ((PWR_CR4_WP2 << PWR_WUP_POLARITY_SHIFT) | PWR_CR3_EWUP2) /*!< Wakeup pin 2 (with low level detection) */
#if defined(PWR_CR3_EWUP3)
#define PWR_WAKEUP_PIN3_LOW                                                                                            \
    ((PWR_CR4_WP3 << PWR_WUP_POLARITY_SHIFT) | PWR_CR3_EWUP3) /*!< Wakeup pin 3 (with low level detection) */
#endif                                                        /* PWR_CR3_EWUP3 */
#define PWR_WAKEUP_PIN4_LOW                                                                                            \
    ((PWR_CR4_WP4 << PWR_WUP_POLARITY_SHIFT) | PWR_CR3_EWUP4) /*!< Wakeup pin 4 (with low level detection) */
#if defined(PWR_CR3_EWUP5)
#define PWR_WAKEUP_PIN5_LOW                                                                                            \
    ((PWR_CR4_WP5 << PWR_WUP_POLARITY_SHIFT) | PWR_CR3_EWUP5) /*!< Wakeup pin 5 (with low level detection) */
#endif                                                        /* PWR_CR3_EWUP5 */
#define PWR_WAKEUP_PIN6_LOW                                                                                            \
    ((PWR_CR4_WP6 << PWR_WUP_POLARITY_SHIFT) | PWR_CR3_EWUP6) /*!< Wakeup pin 6 (with low level detection) */
/**
 * @}
 */
...
#define PWR_SLEEPENTRY_WFI                                                                                             \
    ((uint8_t)0x01u)                        /*!< Wait For Interruption instruction to enter Sleep                      \
                                               mode */
...
#define PWR_STOPENTRY_WFI                                                                                              \
    ((uint8_t)0x01u)                       /*!< Wait For Interruption instruction to enter Stop mode                   \
                                            */
...
#define PWR_FLAG_PVMO_VDDIO2                                                                                           \
    (0x00020000u | PWR_SR2_PVMO_VDDIO2) /*!< Power Voltage Monitoring output                                           \
                                         */
 ...
 *                 can be used or not
 *            @arg PWR_FLAG_REGLPS: Low Power Regulator Started. Indicates
 * whether or not the low-power regulator is ready.
 *            @arg PWR_FLAG_REGLPF: Low Power Regulator Flag. Indicates whether
 * the regulator is ready in main mode or is in low-power mode.
 * @if defined(STM32G081xx)
 *            @arg PWR_FLAG_PVDO: Power Voltage Detector Output. Indicates
 * whether VDD voltage is below or above the selected PVD threshold.
 * @endif
 * @retval The new state of __FLAG__ (TRUE or FALSE).
 */
#define __HAL_PWR_GET_FLAG(__FLAG__)                                                                                   \
    (((__FLAG__) & 0x00010000u) ? ((PWR->SR1 & ((__FLAG__) & ~0x00030000u)) == ((__FLAG__) & ~0x00030000u))            \
...
/* Private constants-------------------------------------------------------*/
/** @defgroup PWR_WUP_Polarity Shift to apply to retrieve polarity information
...
#include "stm32g0xx_hal_pwr_ex.h"
    ...
    /** @defgroup PWR_Exported_Functions_Group1  Initialization and
