...
#include "stm32g0xx_hal_def.h"
    ...
    typedef struct
    {
        uint32_t PVMType; /*!< PVMType: Specifies the VDDIO2 Power Voltage
                             Monitoring type. This parameter can be a value of @ref
                             PWREx_PVM_Type. */
...
#define PWR_PVDLEVEL_0                                                                                                 \
    (PWR_PVDLEVEL_RISING_0 | PWR_PVDLEVEL_FALLING_0) /*!< same PVD threshold level 0 on rising &                       \
                                                        falling */
#define PWR_PVDLEVEL_1                                                                                                 \
    (PWR_PVDLEVEL_RISING_1 | PWR_PVDLEVEL_FALLING_1) /*!< same PVD threshold level 1 on rising &                       \
                                                        falling */
#define PWR_PVDLEVEL_2                                                                                                 \
    (PWR_PVDLEVEL_RISING_2 | PWR_PVDLEVEL_FALLING_2) /*!< same PVD threshold level 2 on rising &                       \
                                                        falling */
#define PWR_PVDLEVEL_3                                                                                                 \
    (PWR_PVDLEVEL_RISING_3 | PWR_PVDLEVEL_FALLING_3) /*!< same PVD threshold level 3 on rising &                       \
                                                        falling */
#define PWR_PVDLEVEL_4                                                                                                 \
    (PWR_PVDLEVEL_RISING_4 | PWR_PVDLEVEL_FALLING_4) /*!< same PVD threshold level 4 on rising &                       \
                                                        falling */
#define PWR_PVDLEVEL_5                                                                                                 \
    (PWR_PVDLEVEL_RISING_5 | PWR_PVDLEVEL_FALLING_5) /*!< same PVD threshold level 5 on rising &                       \
                                                        falling */
#define PWR_PVDLEVEL_6                                                                                                 \
    (PWR_PVDLEVEL_RISING_6 | PWR_PVDLEVEL_FALLING_6) /*!< same PVD threshold level 6 on rising &                       \
                                                        falling */
#define PWR_PVDLEVEL_7                                                                                                 \
    (PWR_CR2_PVDRT_2 | PWR_CR2_PVDRT_1 | PWR_CR2_PVDRT_0) /*!< External input analog voltage (compared internally      \
                                                             to VREFINT) */
/**
 * @}
 */
 ...
 * @{
 */
#define PWR_PVM_DISABLE                                                                                                \
    ((uint32_t)0x00000000) /*!< VDDIO2 monitoring disabled, IOs in isolation                                           \
                              mode */
#define PWR_PVM_ENABLE                                                                                                 \
    ((uint32_t)PWR_CR2_PVM_VDDIO2_0) /*!< VDDIO2 monitoring enabled, IOs                                               \
                                        enabled or in isolation mode,                                                  \
                                        according to VDDIO2 level */
#define PWR_PVM_BYPASS                                                                                                 \
    ((uint32_t)(PWR_CR2_PVM_VDDIO2_1 | PWR_CR2_PVM_VDDIO2_2)) /*!< VDDIO2 monitoring bypassed, IOs                     \
                                                                 enabled */
...
#define PWR_PVM_MODE_IT_RISING_FALLING                                                                                 \
    ((uint32_t)0x00010003) /*!< External Interrupt Mode with Rising/Falling                                            \
 ...
 */
#define PVM_MODE_IT                                                                                                    \
    ((uint32_t)0x00010000) /*!< Mask for interruption yielded by PVM threshold                                         \
                              crossing */
#define PVM_MODE_EVT                                                                                                   \
...
#define IS_PWR_BATTERY_RESISTOR_SELECT(__RESISTOR__)                                                                   \
    (((__RESISTOR__) == PWR_BATTERY_CHARGING_RESISTOR_5) || ((__RESISTOR__) == PWR_BATTERY_CHARGING_RESISTOR_1_5))

#define IS_PWR_GPIO_BIT_NUMBER(__BIT_NUMBER__)                                                                         \
    ((((__BIT_NUMBER__) & 0x0000FFFFu) != 0x00u) && (((__BIT_NUMBER__) & 0xFFFF0000u) == 0x00u))
#if defined(GPIOE)
#define IS_PWR_GPIO(__GPIO__)                                                                                          \
    (((__GPIO__) == PWR_GPIO_A) || ((__GPIO__) == PWR_GPIO_B) || ((__GPIO__) == PWR_GPIO_C) ||                         \
     ((__GPIO__) == PWR_GPIO_D) || ((__GPIO__) == PWR_GPIO_E) || ((__GPIO__) == PWR_GPIO_F))
#else
#define IS_PWR_GPIO(__GPIO__)                                                                                          \
    (((__GPIO__) == PWR_GPIO_A) || ((__GPIO__) == PWR_GPIO_B) || ((__GPIO__) == PWR_GPIO_C) ||                         \
     ((__GPIO__) == PWR_GPIO_D) || ((__GPIO__) == PWR_GPIO_F))
#endif /* GPIOE */

#define IS_PWR_FLASH_POWERDOWN(__MODE__)                                                                               \
...
#define IS_PWR_PVD_MODE(MODE)                                                                                          \
    (((MODE) == PWR_PVD_MODE_NORMAL) || ((MODE) == PWR_PVD_MODE_IT_RISING) || ((MODE) == PWR_PVD_MODE_IT_FALLING) ||   \
     ((MODE) == PWR_PVD_MODE_IT_RISING_FALLING) || ((MODE) == PWR_PVD_MODE_EVENT_RISING) ||                            \
     ((MODE) == PWR_PVD_MODE_EVENT_FALLING) || ((MODE) == PWR_PVD_MODE_EVENT_RISING_FALLING))
#endif /* PWR_PVD_SUPPORT */

#if defined(PWR_PVM_SUPPORT)
#define IS_PWR_PVM_TYPE(TYPE) (((TYPE) == PWR_PVM_DISABLE) || ((TYPE) == PWR_PVM_ENABLE) || ((TYPE) == PWR_PVM_BYPASS))

#define IS_PWR_PVM_MODE(MODE)                                                                                          \
    (((MODE) == PWR_PVM_MODE_NORMAL) || ((MODE) == PWR_PVM_MODE_IT_RISING) || ((MODE) == PWR_PVM_MODE_IT_FALLING) ||   \
     ((MODE) == PWR_PVM_MODE_IT_RISING_FALLING) || ((MODE) == PWR_PVM_MODE_EVENT_RISING) ||                            \
     ((MODE) == PWR_PVM_MODE_EVENT_FALLING) || ((MODE) == PWR_PVM_MODE_EVENT_RISING_FALLING))
#endif /* PWR_PVM_SUPPORT */
    /**
     * @}
     */
