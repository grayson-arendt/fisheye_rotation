...
#include "stm32g0xx_hal_def.h"
    ...
    typedef struct
    {
        uint32_t BaudRate; /*!< This member configures the UART communication baud rate.
                              ...
                              UART input clock divided by a prescaler.
                                @note For UART :
                              ...
                              ((huart->Init.BaudRate)))
                                ...
                                where uart_ker_ck_pres is the UART input clock divided by
                              a prescaler */

        uint32_t WordLength; /*!< Specifies the number of data bits transmitted or
                                received in a frame. This parameter can be a value
                                of @ref UARTEx_Word_Length. */
        ...
        uint32_t Parity; /*!< Specifies the parity mode.
                            ...
                            (9th bit when the word length is set to 9 data bits; 8th
        ...
        uint32_t OneBitSampling; /*!< Specifies whether a single sample or three
                                    samples' majority vote is selected. Selecting
                                    the single sample method increases the receiver
                                    tolerance to clock deviations. This parameter
                                    can be a value of @ref UART_OneBit_Sampling. */

        uint32_t ClockPrescaler; /*!< Specifies the prescaler value used to divide
                                    the UART clock source. This parameter can be a
                                    value of @ref UART_ClockPrescaler. */
    ...
    typedef struct
    {
        ...
        uint32_t DataInvert; /*!< Specifies whether data are inverted
                                (positive/direct logic vs negative/inverted logic).
        ...
        uint32_t DMADisableonRxError; /*!< Specifies whether the DMA is disabled in case
                                         of reception error. This parameter can be a
                                         value of @ref UART_DMA_Disable_on_Rx_Error. */
        ...
        uint32_t AutoBaudRateMode; /*!< If auto Baud rate detection is enabled,
                                      specifies how the rate detection is carried
                                      out. This parameter can be a value of @ref
...
#define HAL_UART_STATE_BUSY_TX_RX                                                                                      \
    0x00000023U /*!< Data Transmission and Reception process is ongoing                                                \
                   ...
                   result of combination (Or) between gState and RxState                                               \
                   values */
 ...
 *         The maximum Baud Rate is derived from the maximum clock on G0 (i.e.
 * 64 MHz) divided by the smallest oversampling used on the USART (i.e. 8)
...
#define IS_UART_STOPBITS(__STOPBITS__)                                                                                 \
    (((__STOPBITS__) == UART_STOPBITS_0_5) || ((__STOPBITS__) == UART_STOPBITS_1) ||                                   \
     ((__STOPBITS__) == UART_STOPBITS_1_5) || ((__STOPBITS__) == UART_STOPBITS_2))
...
#define IS_UART_HARDWARE_FLOW_CONTROL(__CONTROL__)                                                                     \
    (((__CONTROL__) == UART_HWCONTROL_NONE) || ((__CONTROL__) == UART_HWCONTROL_RTS) ||                                \
     ((__CONTROL__) == UART_HWCONTROL_CTS) || ((__CONTROL__) == UART_HWCONTROL_RTS_CTS))
...
#define IS_UART_REQUEST_PARAMETER(__PARAM__)                                                                           \
    (((__PARAM__) == UART_AUTOBAUD_REQUEST) || ((__PARAM__) == UART_SENDBREAK_REQUEST) ||                              \
     ((__PARAM__) == UART_MUTE_MODE_REQUEST) || ((__PARAM__) == UART_RXDATA_FLUSH_REQUEST) ||                          \
...
#define IS_UART_WAKEUP_SELECTION(__WAKE__)                                                                             \
    (((__WAKE__) == UART_WAKEUP_ON_ADDRESS) || ((__WAKE__) == UART_WAKEUP_ON_STARTBIT) ||                              \
...
#define IS_UART_PRESCALER(__CLOCKPRESCALER__)                                                                          \
    (((__CLOCKPRESCALER__) == UART_PRESCALER_DIV1) || ((__CLOCKPRESCALER__) == UART_PRESCALER_DIV2) ||                 \
     ((__CLOCKPRESCALER__) == UART_PRESCALER_DIV4) || ((__CLOCKPRESCALER__) == UART_PRESCALER_DIV6) ||                 \
     ((__CLOCKPRESCALER__) == UART_PRESCALER_DIV8) || ((__CLOCKPRESCALER__) == UART_PRESCALER_DIV10) ||                \
     ((__CLOCKPRESCALER__) == UART_PRESCALER_DIV12) || ((__CLOCKPRESCALER__) == UART_PRESCALER_DIV16) ||               \
     ((__CLOCKPRESCALER__) == UART_PRESCALER_DIV32) || ((__CLOCKPRESCALER__) == UART_PRESCALER_DIV64) ||               \
     ((__CLOCKPRESCALER__) == UART_PRESCALER_DIV128) || ((__CLOCKPRESCALER__) == UART_PRESCALER_DIV256))
