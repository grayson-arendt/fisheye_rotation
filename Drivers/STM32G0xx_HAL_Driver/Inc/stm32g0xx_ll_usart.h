...
#include "stm32g0xx.h"
    ...
    static const uint32_t USART_PRESCALER_TAB[] = {1UL,  2UL,  4UL,   6UL,   8UL,   10UL,  12UL,  16UL,
                                                   32UL, 64UL, 128UL, 256UL, 256UL, 256UL, 256UL, 256UL};
/**
 * @}
 */
...
/* Exported types ------------------------------------------------------------*/
    ...
    typedef struct
    {
        ...
        uint32_t BaudRate; /*!< This field defines expected Usart communication baud
                              rate.

                                This feature can be modified afterwards using
                              unitary function @ref LL_USART_SetBaudRate().*/
        ...
        uint32_t DataWidth; /*!< Specifies the number of data bits transmitted or
                               ...
                               @ref USART_LL_EC_DATAWIDTH.
                                 ...
                                 This feature can be modified afterwards using
                               unitary function @ref LL_USART_SetDataWidth().*/
        ...
        uint32_t StopBits; /*!< Specifies the number of stop bits transmitted.
                              ...
                              USART_LL_EC_STOPBITS.
                                ...
                                This feature can be modified afterwards using
                              unitary function @ref LL_USART_SetStopBitsLength().*/
        ...
        uint32_t TransferDirection; /*!< Specifies whether the Receive and/or Transmit
                                       mode is enabled or disabled. This parameter can be
                                       a value of @ref USART_LL_EC_DIRECTION.
    ...
    typedef struct
    {
        uint32_t ClockOutput; /*!< Specifies whether the USART clock is enabled or
                                 ...
                                 LL_USART_DisableSCLKOutput(). For more details,
        ...
        uint32_t ClockPolarity; /*!< Specifies the steady state of the serial clock.
                                   ...
                                   USART_LL_EC_POLARITY.
                                     ...
                                     USART HW configuration can be modified
                                   afterwards using unitary functions @ref
                                   LL_USART_SetClockPolarity(). For more details,
                                   refer to description of this function. */

        uint32_t ClockPhase; /*!< Specifies the clock transition on which the bit
                                capture is made. This parameter can be a value of
                                @ref USART_LL_EC_PHASE.

                                  USART HW configuration can be modified afterwards
                                using unitary functions @ref
                                LL_USART_SetClockPhase(). For more details, refer to
                                description of this function. */

        uint32_t LastBitClockPulse; /*!< Specifies whether the clock pulse corresponding
                                       to the last transmitted data bit (MSB) has to be
                                       output on the SCLK pin in synchronous mode. This
                                       parameter can be a value of @ref
                                       USART_LL_EC_LASTCLKPULSE.

                                         USART HW configuration can be modified
                                       afterwards using unitary functions @ref
                                       LL_USART_SetLastClkPulseOutput(). For more
...
#define LL_USART_ISR_TCBGT                                                                                             \
    USART_ISR_TCBGT                      /*!< Transmission complete before guard time completion                       \
                                            flag */
...
#define LL_USART_POLARITY_LOW                                                                                          \
    0x00000000U /*!< Steady low value on SCLK pin outside transmission                                                 \
                   window*/
...
#define LL_USART_AUTOBAUD_DETECT_ON_FALLINGEDGE                                                                        \
    USART_CR2_ABRMODE_0                                          /*!< Falling edge to falling edge measurement.        \
                                                                    Received frame must start with a single bit = 1 -> \
                                                                    Frame = Start10xxxxxx */
...
#define LL_USART_HWCONTROL_RTS                                                                                         \
    USART_CR3_RTSE /*!< RTS output enabled, data is only requested when there                                          \
                      is space in the receive buffer */
#define LL_USART_HWCONTROL_CTS                                                                                         \
    USART_CR3_CTSE /*!< CTS mode enabled, data is only transmitted when the                                            \
                      nCTS input is asserted (tied to 0) */
...
#define __LL_USART_DIV_SAMPLING16(__PERIPHCLK__, __PRESCALER__, __BAUDRATE__)                                          \
     ...
     * @retval None
     */
    __STATIC_INLINE void LL_USART_ConfigFIFOsThreshold(USART_TypeDef *USARTx, uint32_t TXThreshold,
    ...
    __STATIC_INLINE uint32_t LL_USART_GetBaudRate(const USART_TypeDef *USARTx, uint32_t PeriphClk,
                                                  uint32_t PrescalerValue, uint32_t OverSampling)
    {
        uint32_t usartdiv;
