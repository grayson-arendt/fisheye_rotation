...
#define HAL_USB_TIMEOUT 0xF000000U
    ...
    typedef struct
    {
        ...
        uint8_t dma_enable; /*!< USB DMA state.
                                  If DMA is not supported this parameter shall be
        ...
        uint8_t bulk_doublebuffer_enable; /*!< Enable or disable the double buffer
                                             mode on bulk EP                  */

        uint8_t iso_singlebuffer_enable; /*!< Enable or disable the Single buffer
                                            mode on Isochronous  EP          */
    } USB_CfgTypeDef;
    ...
    typedef struct
    {
        ...
        uint8_t ch_dir; /*!< channel direction
                             This parameter store the physical channel direction
                           IN/OUT/BIDIR           */

        uint8_t speed; /*!< USB Host Channel speed.
                            This parameter can be any value of @ref
                          HCD_Device_Speed: (HCD_DEVICE_SPEED_xxx)             */
...
#define USB_DRD_CLEAR_CHEP_RX_ERR(USBx, bChNum)                                                                        \
    ...
    {                                                                                                                  \
        ...
        _wRegVal =                                                                                                     \
            (_wRegVal & USB_CHEP_REG_MASK & (~USB_CHEP_ERRRX) & (~USB_CHEP_VTRX)) | (USB_CHEP_VTTX | USB_CHEP_ERRTX);  \
                                                                                                                       \
...
#define USB_DRD_CLEAR_CHEP_TX_ERR(USBx, bChNum)                                                                        \
    ...
    {                                                                                                                  \
        ...
        _wRegVal = USB_DRD_GET_CHEP((USBx), (bChNum));                                                                 \
        _wRegVal =                                                                                                     \
            (_wRegVal & USB_CHEP_REG_MASK & (~USB_CHEP_ERRTX) & (~USB_CHEP_VTTX)) | (USB_CHEP_VTRX | USB_CHEP_ERRRX);  \
                                                                                                                       \
        USB_DRD_SET_CHEP((USBx), (bChNum), _wRegVal);                                                                  \
...
#define USB_DRD_SET_CHEP_ADDRESS(USBx, bEpChNum, bAddr)                                                                \
    ...
    {                                                                                                                  \
        uint32_t _wRegVal;                                                                                             \
                                                                                                                       \
        /*Read the USB->CHEPx into _wRegVal, Reset(DTOGRX/STRX/DTOGTX/STTX)                                            \
         * and set the EpAddress*/                                                                                     \
        _wRegVal = (USB_DRD_GET_CHEP((USBx), (bEpChNum)) & USB_CHEP_REG_MASK) | (bAddr);                               \
                                                                                                                       \
        /*Set _wRegVal in USB->CHEPx and set Transmit/Receive Valid Transfer                                           \
         * (x=bEpChNum)*/                                                                                              \
        USB_DRD_SET_CHEP((USBx), (bEpChNum), (_wRegVal | USB_CHEP_VTRX | USB_CHEP_VTTX));                              \
 ...
 * ------------------------------------------------------------*/
/* Buffer Descriptor Table   TXBD0/RXBD0 --- > TXBD7/RXBD7  8 possible
 * descriptor The buffer descriptor is located inside the packet buffer memory
 * (USB_PMA_BUFF) TXBD    [Reserve         |Countx| Address_Tx] RXBD
