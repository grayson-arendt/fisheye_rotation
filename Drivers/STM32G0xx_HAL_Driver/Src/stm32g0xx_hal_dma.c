...
HAL_StatusTypeDef HAL_DMA_Init(DMA_HandleTypeDef *hdma)
{
    ...
    CLEAR_BIT(hdma->Instance->CCR, (DMA_CCR_PL | DMA_CCR_MSIZE | DMA_CCR_PSIZE | DMA_CCR_MINC | DMA_CCR_PINC |
                                    DMA_CCR_CIRC | DMA_CCR_DIR | DMA_CCR_MEM2MEM));

    /* Set the DMA Channel configuration */
    SET_BIT(hdma->Instance->CCR,
            (hdma->Init.Direction | hdma->Init.PeriphInc | hdma->Init.MemInc | hdma->Init.PeriphDataAlignment |
    ...
    */
    ...
    if (((hdma->Init.Request > 0UL) && (hdma->Init.Request <= DMA_REQUEST_GENERATOR3)))
    {
        /* Initialize parameters for DMAMUX request generator :
           DMAmuxRequestGen, DMAmuxRequestGenStatus and
                                   ...
                                   uint32_t DataLength)
{
    ...
    if (hdma->State == HAL_DMA_STATE_READY)
    {
        ...
        {
            /* if using DMAMUX request generator, enable the DMAMUX request
             * generator overrun IT*/
...
HAL_StatusTypeDef HAL_DMA_Abort(DMA_HandleTypeDef *hdma)
{
    ...
    else
    {
        ...
        if (hdma->DMAmuxRequestGen != 0U)
        {
            /* if using DMAMUX request generator, disable the DMAMUX request
             * generator overrun IT*/
...
static void DMA_CalcDMAMUXRequestGenBaseAndMask(DMA_HandleTypeDef *hdma)
{
    ...
    /* here "Request" is either DMA_REQUEST_GENERATOR0 to
     * DMA_REQUEST_GENERATOR3, i.e. <= 4*/
