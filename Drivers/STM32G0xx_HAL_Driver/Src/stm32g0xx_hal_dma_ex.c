...
then be used to respectively enable/disable the request generator.
    (+) Handle DMAMUX interrupts using HAL_DMAEx_MUX_IRQHandler : should be
called from the DMAMUX IRQ handler
...
HAL_StatusTypeDef HAL_DMAEx_ConfigMuxSync(DMA_HandleTypeDef *hdma, const HAL_DMA_MuxSyncConfigTypeDef *pSyncConfig)
{
    ...
    if (hdma->State == HAL_DMA_STATE_READY)
    {
        ...
        /* Set the new synchronization parameters (and keep the request ID
         * filled during the Init)*/
    ...
    DMA_HandleTypeDef *hdma, const HAL_DMA_MuxRequestGeneratorConfigTypeDef *pRequestGeneratorConfig)
{
    ...
    */
    ...
    {
        /* RequestGenerator must be disable prior to the configuration i.e GE
         * bit is 0 */
