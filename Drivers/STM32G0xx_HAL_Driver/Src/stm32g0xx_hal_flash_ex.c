/**
 ...
 ==============================================================================
  ...
  [..] Comparing to other previous devices, the FLASH interface for STM32G0xx
       ...
       (+) Capacity up to 128 Kbytes with single bank architecture supporting
 ...
 to :
      ...
      (#) Check is flash content is empty or not using
 HAL_FLASHEx_FlashEmptyCheck. and modify this setting (for flash loader purpose
 e.g.) using HAL_FLASHEx_ForceFlashEmpty.

      (#) Enable securable memory area protectionusing
 ...
 * @param[in]  pEraseInit Pointer to an @ref FLASH_EraseInitTypeDef structure
        ...
        else
        {
            ...
            *PageError = 0xFFFFFFFFU;
            ...
            for (index = pEraseInit->Page; index < (pEraseInit->Page + pEraseInit->NbPages); index++)
            {
                ...
                if (status != HAL_OK)
                {
                    /* In case of error, stop erase procedure and return the
                     * faulty address */
                    *PageError = index;
                    break;
                }
            ...
            /* If operation is completed or interrupted, disable the Page Erase
             * Bit */
 ...
 *         - after an option bytes launch through the call of @ref
 * HAL_FLASH_OB_Launch()
 *         - a Power On Reset
...
HAL_StatusTypeDef HAL_FLASHEx_OBProgram(FLASH_OBProgramInitTypeDef *pOBInit)
{
    ...
    if (status == HAL_OK)
    {
        ...
        /* If the option byte program operation is completed, disable the
         * OPTSTRT Bit */
 ...
 * @param  pOBInit Pointer to an @ref FLASH_OBProgramInitTypeDef structure that
 * contains the configuration information. The fields pOBInit->WRPArea should
 ...
 * @note   After calling this API, Debugger is disabled: it is no more possible
 ...
 * @note   On some devices, there is only 1 bank so parameter has to be set
 * FLASH_BANK_1.
 * @note   This API locks Securable memory area which is defined in SEC_SIZE
 * option byte (that can be retrieved calling HAL_FLASHEx_OBGetConfig API and
 * checking Secsize).
 ...
 * number of pages in the Flash Bank - 1)
 * @retval None
 */
static void FLASH_OB_WRPConfig(uint32_t WRPArea, uint32_t WRPStartOffset, uint32_t WRDPEndOffset)
{
 ...
 * @param[out]  WRDPEndOffset  Dpecifies the address where to copied the end
 * page of the write protected area
 * @retval None
 */
static void FLASH_OB_GetWRP(uint32_t WRPArea, uint32_t *WRPStartOffset, uint32_t *WRDPEndOffset)
 ...
 * @retval The FLASH User Option Bytes values. It will be a combination of all
...
#if defined(FLASH_PCROP_SUPPORT)
/**
 * @brief  Configure the 1A Proprietary code readout protection & erase
 * configuration on RDP regression.
 * @note   It is recommended to align PCROP zone with page granularity when
 * using PCROP_RDP or avoid having some executable code in a page where PCROP
 * zone starts or ends.
 * @note   Minimum PCROP area size is 2 times the chosen granularity:
 * PCROPA_STRT and PCROPA_END. So if the requirement is to be able to
 * read-protect 1KB areas, the ROP granularity has to be set to 512 Bytes
 * @param  PCROPConfig  specifies the erase configuration
 * (OB_PCROP_RDP_NOT_ERASE or OB_PCROP_RDP_ERASE) on RDP level 1 regression.
 * @param  PCROP1AStartAddr Specifies the Zone 1A Start address of the
 * Proprietary code readout protection This parameter can be an address between
 * begin and end of the flash
 * @param  PCROP1AEndAddr Specifies the Zone 1A end address of the Proprietary
 * code readout protection This parameter can be an address between
 * PCROP1AStartAddr and end of the flash
 * @retval None
 */
static void FLASH_OB_PCROP1AConfig(uint32_t PCROPConfig, uint32_t PCROP1AStartAddr, uint32_t PCROP1AEndAddr)
{
    ...
    /* Set RDP erase protection if needed. This bit is only set & will be reset
     * by mass erase */
 ...
 * zone starts or ends.
 * @note   Minimum PCROP area size is 2 times the chosen granularity:
 * PCROPB_STRT and PCROPB_END. So if the requirement is to be able to
 * read-protect 1KB areas, the ROP granularity has to be set to 512 Bytes
 * @param  PCROP1BStartAddr  Specifies the Zone 1B Start address of the
 * Proprietary code readout protection This parameter can be an address between
 * begin and end of the flash
 * @param  PCROP1BEndAddr  Specifies the Zone 1B end address of the Proprietary
 * code readout protection This parameter can be an address between
 * PCROP1BStartAddr and end of the flash
 * @retval None
 */
static void FLASH_OB_PCROP1BConfig(uint32_t PCROP1BStartAddr, uint32_t PCROP1BEndAddr)
{
 ...
 * @param  PCROP1AStartAddr [out]  Specifies the address where to copied the
 * start address of the 1A Proprietary code readout protection
 * @param  PCROP1AEndAddr [out]  Specifies the address where to copied the end
 * address of the 1A Proprietary code readout protection
 * @retval None
 */
static void FLASH_OB_GetPCROP1A(uint32_t *PCROPConfig, uint32_t *PCROP1AStartAddr, uint32_t *PCROP1AEndAddr)
{
...
/**
 * @brief  Return the FLASH PCROP Protection Option Bytes value.
 * @param  PCROP1BStartAddr [out]  Specifies the address where to copied the
 * start address of the 1B Proprietary code readout protection
 * @param  PCROP1BEndAddr [out]  Specifies the address where to copied the end
 * address of the 1B Proprietary code readout protection
 * @retval None
 */
static void FLASH_OB_GetPCROP1B(uint32_t *PCROP1BStartAddr, uint32_t *PCROP1BEndAddr)
{
 ...
 * PCROPA_STRT and PCROPA_END. So if the requirement is to be able to
 * read-protect 1KB areas, the ROP granularity has to be set to 512 Bytes
 * @param  PCROP2AStartAddr  Specifies the Zone 2A Start address of the
 * Proprietary code readout protection This parameter can be an address between
 * begin and end of the flash
 * @param  PCROP2AEndAddr  Specifies the Zone 2A end address of the Proprietary
 * code readout protection This parameter can be an address between
 * PCROP2AStartAddr and end of the flash
 * @retval None
 */
static void FLASH_OB_PCROP2AConfig(uint32_t PCROP2AStartAddr, uint32_t PCROP2AEndAddr)
{
 ...
 * using PCROP_RDP or avoid having some executable code in a page where PCROP
 * zone starts or ends.
 * @note   Minimum PCROP area size is 2 times the chosen granularity: PCROP_STRT
 * and PCROP_END. So if the requirement is to be able to read-protect 1KB areas,
 * the ROP granularity has to be set to 512 Bytes
 * @param  PCROP2BStartAddr  Specifies the Zone 2B Start address of the
 * Proprietary code readout protection This parameter can be an address between
 * begin and end of the flash
 * @param  PCROP2BEndAddr  Specifies the Zone 2B end address of the Proprietary
 * code readout protection This parameter can be an address between
 * PCROP2BStartAddr and end of the flash
 * @retval None
 */
static void FLASH_OB_PCROP2BConfig(uint32_t PCROP2BStartAddr, uint32_t PCROP2BEndAddr)
{
...
/**
 * @brief  Return the FLASH PCROP Protection Option Bytes value.
 * @param  PCROP2AStartAddr [out]  Specifies the address where to copied the
 * start address of the 2A Proprietary code readout protection
 * @param  PCROP2AEndAddr [out]  Specifies the address where to copied the end
 * address of the 2A Proprietary code readout protection
 * @retval None
 */
static void FLASH_OB_GetPCROP2A(uint32_t *PCROP2AStartAddr, uint32_t *PCROP2AEndAddr)
{
...
/**
 * @brief  Return the FLASH PCROP Protection Option Bytes value.
 * @param  PCROP2BStartAddr [out]  Specifies the address where to copied the
 * start address of the 2B Proprietary code readout protection
 * @param  PCROP2BEndAddr [out]  Specifies the address where to copied the end
 * address of the 2B Proprietary code readout protection
 * @retval None
 */
static void FLASH_OB_GetPCROP2B(uint32_t *PCROP2BStartAddr, uint32_t *PCROP2BEndAddr)
{
 ...
 * @param  BootEntry  specifies if boot scheme is forced to Flash (System or
 * user) or not This parameter can be one of the following values:
 *           @arg @ref OB_BOOT_ENTRY_FORCED_NONE No boot entry forced
 *           @arg @ref OB_BOOT_ENTRY_FORCED_FLASH Flash selected as unique entry
 * boot
 * @param  SecSize specifies number of pages to protect as securable memory
 * area, starting from beginning of Bank1 (page 0).
 * @param  SecSize2 specifies number of pages to protect as securable memory
 * area, starting from beginning of Bank2 (page 0).
 * @retval None
 */
static void FLASH_OB_SecMemConfig(uint32_t BootEntry, uint32_t SecSize, uint32_t SecSize2)
{
 ...
 * @param  SecSize specifies number of pages to protect as secure memory area,
 * starting from beginning of Bank1 (page 0).
 * @param  SecSize2 specifies number of pages to protect as secure memory area,
 * starting from beginning of Bank2 (page 0).
 * @retval None
 */
static void FLASH_OB_GetSecMem(uint32_t *BootEntry, uint32_t *SecSize, uint32_t *SecSize2)
{
 ...
 * boot
 * @param  SecSize specifies number of pages to protect as securable memory
 * area, starting from beginning of the Flash (page 0).
 * @retval None
 */
static void FLASH_OB_SecMemConfig(uint32_t BootEntry, uint32_t SecSize)
{
 ...
 * @param  BootEntry  specifies boot scheme configuration
 * @param  SecSize specifies number of pages to protect as secure memory area,
 * starting from beginning of the Flash (page 0).
 * @retval None
 */
static void FLASH_OB_GetSecMem(uint32_t *BootEntry, uint32_t *SecSize)
{
