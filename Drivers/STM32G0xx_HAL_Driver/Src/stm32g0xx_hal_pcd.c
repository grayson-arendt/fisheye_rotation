...
HAL_StatusTypeDef HAL_PCD_UnRegisterIsoOutIncpltCallback(PCD_HandleTypeDef *hpcd)
{
    ...
    if (hpcd->State == HAL_PCD_STATE_READY)
    {
        hpcd->ISOOUTIncompleteCallback = HAL_PCD_ISOOUTIncompleteCallback; /* Legacy weak
...
HAL_StatusTypeDef HAL_PCD_UnRegisterIsoInIncpltCallback(PCD_HandleTypeDef *hpcd)
{
    ...
    if (hpcd->State == HAL_PCD_STATE_READY)
    {
        hpcd->ISOINIncompleteCallback = HAL_PCD_ISOINIncompleteCallback; /* Legacy weak
...
void HAL_PCD_IRQHandler(PCD_HandleTypeDef *hpcd)
{
    ...
    if ((SYSCFG->IT_LINE_SR[8] & (0x1UL << 2)) == 0U)
    {
        return;
    }
...
__weak void HAL_PCD_SetupStageCallback(PCD_HandleTypeDef *hpcd)
{
    ...
    /* NOTE : This function should not be modified, when the callback is needed,
              the HAL_PCD_SetupStageCallback could be implemented in the user
       file
     */
}
...
__weak void HAL_PCD_ISOINIncompleteCallback(PCD_HandleTypeDef *hpcd, uint8_t epnum)
{
    ...
    /* NOTE : This function should not be modified, when the callback is needed,
              the HAL_PCD_ISOINIncompleteCallback could be implemented in the
       user file
     */
}
...
__weak void HAL_PCD_DisconnectCallback(PCD_HandleTypeDef *hpcd)
{
    ...
    /* NOTE : This function should not be modified, when the callback is needed,
              the HAL_PCD_DisconnectCallback could be implemented in the user
       file
     */
}
                    ...
                    else
                    {
                        if (ep->xfer_buff != 0U)
                        {
                            USB_ReadPMA(hpcd->Instance, ep->xfer_buff, ep->pmaadress,
                                        (uint16_t)ep->xfer_count); /* max 64bytes */
