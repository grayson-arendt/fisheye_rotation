 ...
 ===============================================================================
    ...
    *** Low Power mode configuration ***
    ...
    [..]
      ...
      (+) Low-power Sleep mode: Cortex-M0+ core stopped, peripherals kept
running and regulator in low power mode.
   ...
   *** Sleep mode / Low-power sleep mode ***
   =========================================
    [..]
      ...
      (+) WFI Exit:
        (++) Any interrupt enabled in nested vectored interrupt controller
(NVIC)
      (+) WFE Exit:
...
mode):
    ...
    [..]  When exiting Stop, the MCU is either in Run mode or in Low-power Run
mode depending on the LPR bit setting.

   *** Standby mode ***
   ====================
    [..] In Standby mode, it is possible to keep backup SRAM content (defined as
         full SRAM) keeping low power regulator on. This is achievable by
setting Ram retention bit calling HAL_PWREx_EnableSRAMRetention API. This
increases power consumption. Its also possible to define I/O states using APIs:
...
API, by setting SLEEPDEEP in Cortex control register.
      (+) Exit:
   ...
   ======================
    [..]
      In Shutdown mode,
        ...
        registers.
      (+) Entry:
          (++) The Shutdown mode is entered through
HAL_PWREx_EnterSHUTDOWNMode() API, by setting SLEEPDEEP in Cortex control
register.
      (+) Exit:
          (++) WKUP pin edge detection, RTC event (wakeup, alarm, timestamp),
               tamper event (internal & external), LSE CSS detection, reset on
               NRST pin.
    [..] Exiting Shutdown generates a brown out reset: Cortex is reset and
execute Reset handler vector, all registers are set to their reset value but
ones in backup domain.
 ...
 * @note   Sram content can be kept setting RRS through
