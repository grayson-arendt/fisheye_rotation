 ...
 */
#define PVD_MODE_IT                                                                                                    \
    0x00010000U /*!< Mask for interruption yielded                                                                     \
                     by PVD threshold crossing     */
#define PVD_MODE_EVT                                                                                                   \
    0x00020000U /*!< Mask for event yielded                                                                            \
                     by PVD threshold crossing     */
#define PVD_RISING_EDGE                                                                                                \
    0x00000001U /*!< Mask for rising edge set as                                                                       \
                     PVD trigger                   */
#define PVD_FALLING_EDGE                                                                                               \
    0x00000002U /*!< Mask for falling edge set as                                                                      \
                     PVD trigger                   */
/**
 * @}
 */
 ...
 * @{
 */
#define PWR_REGLPF_SETTING_DELAY_6_US                                                                                  \
    6u /*!< REGLPF should rise in about 5 us plus                                                                      \
            2 APB clock. Taking in account max Sysclk at                                                               \
            2 MHz, and rounded to upper value */

#define PWR_VOSF_SETTING_DELAY_6_US                                                                                    \
    6u /*!< VOSF should rise in about 5 us plus                                                                        \
            2 APB clock. Taking in account max Sysclk at                                                               \
 ...
 * @note  Remove VDDUSB electrical and logical isolation, once VDDUSB supply is
 * present.
 * @note  For STM32G0C1xx and STM32G0B1xx devices, this function is deprecated
 * and retained only to ensure backward compatibility. Power Voltage Monitoring
 * configuration is now handled by the HAL_PWREx_ConfigPVM() API. The USV, IOSV,
 * and PVMENUSB bits have been merged into the PVM_VDDIO2 bitfield of the
 * PWR_CR2 register. Please refer to RM0444 Rev6 or later, section "Power
 ...
 * configuration is now handled by the HAL_PWREx_ConfigPVM() API. The USV, IOSV,
 * and PVMENUSB bits have been merged into the PVM_VDDIO2 bitfield of the
 * PWR_CR2 register. Please refer to RM0444 Rev 6 or later, section "Power
 * control register 2 (PWR_CR2)".
 * @retval None
 */
 ...
 * @note  Remove VDDIO2 electrical and logical isolation, once VDDIO2 supply is
 * present.
 * @note  For STM32G0C1xx and STM32G0B1xx devices, this function is deprecated
 * and retained only to ensure backward compatibility. Power Voltage Monitoring
 * configuration is now handled by the HAL_PWREx_ConfigPVM() API. The USV, IOSV,
...
/**
 * @brief Enable the Power Voltage Monitoring for USB peripheral (power domain
 * Vddio2)
 * @note  For STM32G0C1xx and STM32G0B1xx devices, this function is deprecated
 * and retained only to ensure backward compatibility. Power Voltage Monitoring
 * configuration is now handled by the HAL_PWREx_ConfigPVM() API. The USV, IOSV,
...
/**
 * @brief Disable the Power Voltage Monitoring for USB peripheral (power domain
 * Vddio2)
 * @note  For STM32G0C1xx and STM32G0B1xx devices, this function is deprecated
 * and retained only to ensure backward compatibility. Power Voltage Monitoring
 * configuration is now handled by the HAL_PWREx_ConfigPVM() API. The USV, IOSV,
...
HAL_StatusTypeDef HAL_PWREx_ConfigPVM(PWR_PVMTypeDef const *sConfigPVM)
{
    ...
    case PWR_PVM_ENABLE:
        ...
        /* Clear any previous config. Keep it clear if no event or IT mode
         * is selected */
 ...
 * @note   Set the relevant PUy bit of PWR_PUCRx register to configure the I/O
 ...
 * PWR_GPIO_F to select the GPIO peripheral.
 ...
 * @note   Reset the relevant PUy bit of PWR_PUCRx register used to configure
 ...
 * @note   Set the relevant PDy bit of PWR_PDCRx register to configure the I/O
 ...
 * PWR_GPIO_A..PWR_GPIO_F to select the GPIO peripheral.
 * @param  GPIONumber Specify the I/O pins numbers.
 *         This parameter can be one of the following values:
 *         PWR_GPIO_BIT_0, ..., PWR_GPIO_BIT_15 (except for ports where less
 *         I/O pins are available) or the logical OR of several of them to set
 ...
 * @note   Reset the relevant PDy bit of PWR_PDCRx register used to configure
 ...
 * HAL_PWREx_EnableGPIOPullDown() APIs ensure there is no conflict when setting
 * PUy or PDy bit.
 * @retval None
 */
 ...
 * range 1 mode, typical output voltage at 1.2 V, system frequency up to 64 MHz.
 *          @arg @ref PWR_REGULATOR_VOLTAGE_SCALE2 Regulator voltage output
 * range 2 mode, typical output voltage at 1.0 V, system frequency up to 16 MHz.
 * @note  When moving from Range 1 to Range 2, the system frequency must be
 * decreased to a value below 16 MHz before calling
 * HAL_PWREx_ControlVoltageScaling() API. When moving from Range 2 to Range 1,
 * the system frequency can be increased to a value up to 64 MHz after calling
 * HAL_PWREx_ControlVoltageScaling() API.
...
HAL_StatusTypeDef HAL_PWREx_ControlVoltageScaling(uint32_t VoltageScaling)
{
    ...
    /* In case of Range 1 selected, we need to ensure that main regulator
     * reaches new value */
...
/**
  ...
  * @note   In Shutdown mode, the PLL, the HSI, the LSI and the HSE oscillators
  are switched
  *         off. The voltage regulator is disabled and Vcore domain is powered
  off.
  *         SRAM and registers contents are lost except for registers in the
  Backup domain.
...
__weak void HAL_PWREx_PVD_PVM_Rising_Callback(void)
{
    /* NOTE : This function should not be modified; when the callback is needed,
              the HAL_PWR_PVD_Rising_Callback can be implemented in the user
       file
    */
}
...
{
    /* NOTE : This function should not be modified; when the callback is needed,
              the HAL_PWR_PVD_Falling_Callback can be implemented in the user
       file
    */
}
