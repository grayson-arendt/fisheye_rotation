  ...
  *
  ...
  ==============================================================================
    [..]
      ...
      (from 8 MHz to reach 16MHz) with Flash 0 wait state. Flash prefetch
  buffer, D-Cache and I-Cache are disabled, and all peripherals are off except
...
/** @defgroup RCC_Exported_Functions_Group1 Initialization and de-initialization
 ...
 clocks:
           (++) The first output (R) is used to generate the high speed system
 clock (up to 64MHz).
           (++) The second output(Q) is used to generate the clock for the
 random analog generator and HStim.
           (++) The Third output (P) is used to generate the clock for the
 Analog to Digital Converter and I2S.

         (+) CSS (Clock security system): once enabled, if a HSE or LSE clock
 failure occurs (HSE used directly or through PLL as System clock source), the
 System clock is automatically switched respectively to HSI or LSI and an
 interrupt is generated if enabled. The interrupt is linked to the Cortex-M0+
 NMI (Non-Maskable Interrupt) exception vector.

         (+) MCOx (microcontroller clock output):
             (++) MCO1 used to output LSI, HSI48(*), HSI, LSE, HSE or main PLL
 clock (through a configurable prescaler) on PA8 pin.
             (++) MCO2(*) used to output LSI, HSI48(*), HSI, LSE, HSE, main PLLR
 clock, PLLQ clock, PLLP clock, RTC clock or RTC_Wakeup (through a configurable
 prescaler) on PA10 pin.
                 (*) available on certain devices only

    [..] System, AHB and APB buses clocks configuration
         (+) Several clock sources can be used to drive the System clock
 (SYSCLK): HSI, HSE, LSI, LSE and main PLL. The AHB clock (HCLK) is derived from
 System clock through configurable prescaler and used to clock the CPU, memory
 and peripherals mapped on AHB bus (DMA, GPIO...).and APB (PCLK1) clock is
 ...
 * Off first and then to HSE On or HSE Bypass.
            ...
            (temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_HSI))
        {
            ...
            else
            {
                ...
                if (temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_HSI)
                {
                    ...
                    /* Update the SystemCoreClock global variable with HSISYS
                     * value  */
    ...
    if (RCC_OscInitStruct->PLL.PLLState != RCC_PLL_NONE)
    {
        ...
        if (__HAL_RCC_GET_SYSCLK_SOURCE() != RCC_SYSCLKSOURCE_STATUS_PLLCLK)
        {
            if (RCC_OscInitStruct->PLL.PLLState == RCC_PLL_ON)
            {
                ...
                /* Configure the main PLL clock source, multiplication and
                 * division factors. */
#if defined(RCC_PLLQ_SUPPORT)
            ...
            else
            {
                ...
                /* Unselect main PLL clock source and disable main PLL outputs
                 * to save power */
#if defined(RCC_PLLQ_SUPPORT)
        ...
        {
            /* Check if there is a request to disable the PLL used as System
             * clock source */
            ...
            {
                /* Do not return HAL_ERROR if request repeats the current
                 * configuration */
 ...
 * @param  RCC_ClkInitStruct  pointer to a @ref RCC_ClkInitTypeDef structure
 * that contains the configuration information for the RCC peripheral.
 ...
 * Reset, the HSI frequency is set to 8 Mhz, then it reaches its default value
 * 16 MHz.
 *
 ...
 *
 * @note   You can use @ref HAL_RCC_GetClockConfig() function to know which
 * clock is currently used as system clock source.
 *
...
HAL_StatusTypeDef HAL_RCC_ClockConfig(const RCC_ClkInitTypeDef *RCC_ClkInitStruct, uint32_t FLatency)
{
    ...
    if (FLatency > __HAL_FLASH_GET_LATENCY())
    {
        ...
        /* Check that the new number of wait states is taken into account to
        access the Flash memory by polling the FLASH_ACR register */
        tickstart = HAL_GetTick();
    ...
    {
        /* Set the highest APB divider in order to ensure that we do not go
    ...
    /* Update the SystemCoreClock global variable */
    SystemCoreClock =
        (HAL_RCC_GetSysClockFreq() >> ((AHBPrescTable[(RCC->CFGR & RCC_CFGR_HPRE) >> RCC_CFGR_HPRE_Pos]) & 0x1FU));
...
/**
 * @brief  Select the clock source to output on MCO1 pin(PA8) or MC02 pin
 * (PA10)(*).
 ...
 *            @arg @ref RCC_MCO_PD10 Clock source to output on MCO1
 * pin(PD10)(*).
 *            @arg @ref RCC_MCO_PF2  Clock source to output on MCO1 pin(PF2)(*).
 *            @arg @ref RCC_MCO_PA10 Clock source to output on MCO2
 * pin(PA10)(*).
 *            @arg @ref RCC_MCO_PA15 Clock source to output on MCO2
 * pin(PA15)(*).
 ...
 *            @arg @ref RCC_MCO1SOURCE_PLLCLK   main PLLR clock selected as MCO
 * source
 *            @arg @ref RCC_MCO1SOURCE_LSI      LSI clock selected as MCO source
 *            @arg @ref RCC_MCO1SOURCE_LSE      LSE clock selected as MCO source
 *            @arg @ref RCC_MCO1SOURCE_PLLPCLK  PLLP clock selected as MCO1
 * source(*)
 *            @arg @ref RCC_MCO1SOURCE_PLLQCLK  PLLQ clock selected as MCO1
 * source(*)
 *            @arg @ref RCC_MCO1SOURCE_RTCCLK   RTC clock selected as MCO1
 * source(*)
 *            @arg @ref RCC_MCO1SOURCE_RTC_WKUP RTC_Wakeup selected as MCO1
 * source(*)
 *            @arg @ref RCC_MCO2SOURCE_NOCLOCK  MCO2 output disabled, no clock
 * on MCO2(*)
 *            @arg @ref RCC_MCO2SOURCE_SYSCLK   system  clock selected as MCO2
 * source(*)
 *            @arg @ref RCC_MCO2SOURCE_HSI48    HSI48 clock selected as MCO2
 * source for devices with HSI48(*)
 *            @arg @ref RCC_MCO2SOURCE_HSI      HSI clock selected as MCO2
 * source(*)
 *            @arg @ref RCC_MCO2SOURCE_HSE      HSE clock selected as MCO2
 * source(*)
 *            @arg @ref RCC_MCO2SOURCE_PLLCLK   main PLLR clock selected as MCO2
 * source(*)
 *            @arg @ref RCC_MCO2SOURCE_LSI      LSI clock selected as MCO2
 * source(*)
 *            @arg @ref RCC_MCO2SOURCE_LSE      LSE clock selected as MCO2
 * source(*)
 *            @arg @ref RCC_MCO2SOURCE_PLLPCLK  PLLP clock selected as MCO2
 * source(*)
 *            @arg @ref RCC_MCO2SOURCE_PLLQCLK  PLLQ clock selected as MCO2
 * source(*)
 *            @arg @ref RCC_MCO2SOURCE_RTCCLK   RTC clock selected as MCO2
 * source(*)
 *            @arg @ref RCC_MCO2SOURCE_RTC_WKUP RTC_Wakeup selected as MCO2
 * source(*)
 ...
 *            @arg @ref RCC_MCO2DIV_128  division by 128 applied to MCO2
 * clock(*)
 *            @arg @ref RCC_MCO2DIV_256  division by 256 applied to MCO2
 * clock(*)
 *            @arg @ref RCC_MCO2DIV_512  division by 512 applied to MCO2
 * clock(*)
 *            @arg @ref RCC_MCO2DIV_1024 division by 1024 applied to MCO2
 * clock(*)
 *
...
void HAL_RCC_MCOConfig(uint32_t RCC_MCOx, uint32_t RCC_MCOSource, uint32_t RCC_MCODiv)
{
    ...
    if (mcoindex == RCC_MCO1_INDEX)
    {
        ...
        /* Mask MCOSEL[] and MCOPRE[] bits then set MCO clock source and
         * prescaler */
 ...
 * @note     If SYSCLK source is LSI, function returns values based on
 * LSI_VALUE(***)
 * @note     If SYSCLK source is LSE, function returns values based on
 * LSE_VALUE(****)
 * @note     (*) HSI_VALUE is a constant defined in stm32g0xx_hal_conf.h file
 * (default value 16 MHz) but the real value may vary depending on the
 * variations in voltage and temperature.
 * @note     (**) HSE_VALUE is a constant defined in stm32g0xx_hal_conf.h file
 * (default value 8 MHz), user has to ensure that HSE_VALUE is same as the real
 *                frequency of the crystal used. Otherwise, this function may
 *                have wrong result.
 * @note     (***) LSE_VALUE is a constant defined in stm32g0xx_hal_conf.h file
 * (default value 32768 Hz).
 * @note     (****) LSI_VALUE is a constant defined in stm32g0xx_hal_conf.h file
 ...
 *         allowing the MCU to perform rescue operations. The CSSI is linked to
 *         the Cortex-M0+ NMI (Non-Maskable Interrupt) exception vector.
 * @note   The LSE Clock Security System Detection bit (LSECSSD in BDCR) can
 * only be cleared by a backup domain reset.
 * @retval None
 */
...
__weak void HAL_RCC_CSSCallback(void)
{
    /* NOTE : This function should not be modified, when the callback is needed,
              the @ref HAL_RCC_CSSCallback should be implemented in the user
       file
     */
}
