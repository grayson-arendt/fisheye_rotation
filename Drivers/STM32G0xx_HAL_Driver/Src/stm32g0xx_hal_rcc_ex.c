 ...
 * specified parameters in the @ref RCC_PeriphCLKInitTypeDef.
 * @param  PeriphClkInit  pointer to a @ref RCC_PeriphCLKInitTypeDef structure
 * that contains a field PeriphClockSelection which can be a combination of the
 * following values:
 ...
 * @note   Care must be taken when @ref HAL_RCCEx_PeriphCLKConfig() is used to
 * select the RTC clock source: in this case the access to Backup domain is
 * enabled.
 *
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_RCCEx_PeriphCLKConfig(RCC_PeriphCLKInitTypeDef const *PeriphClkInit)
{
    ...
    if ((PeriphClkInit->PeriphClockSelection & RCC_PERIPHCLK_RTC) == RCC_PERIPHCLK_RTC)
    {
        ...
        if (ret == HAL_OK)
        {
            ...
            {
                /* Store the content of BDCR register before the reset of Backup
                 * Domain */
                tmpregister = READ_BIT(RCC->BDCR, ~(RCC_BDCR_RTCSEL));
                /* RTC Clock selection can be changed only if the Backup Domain
                 * is reset */
            ...
            /* Wait for LSE reactivation if LSE was enable prior to Backup
             * Domain reset */
 ...
 *         LPTIM1 (1), LPTIM2 (1), TIM1 (2), TIM15 (1)(2), USART2 (2), LPUART1
 * (1), CEC (1) and RNG (1)
...
void HAL_RCCEx_GetPeriphCLKConfig(RCC_PeriphCLKInitTypeDef *PeriphClkInit)
{
    ...
    PeriphClkInit->PeriphClockSelection =
        RCC_PERIPHCLK_USART1 | RCC_PERIPHCLK_I2C1 | RCC_PERIPHCLK_I2S1 | RCC_PERIPHCLK_ADC | RCC_PERIPHCLK_RTC;
        ...
        case RCC_PERIPHCLK_ADC:
            ...
            else if (srcclk == RCC_ADCCLKSOURCE_PLLADC)
            {
                if (__HAL_RCC_GET_PLLCLKOUT_CONFIG(RCC_PLLPCLK) != 0U)
                {
                    ...
                    plln = READ_BIT(RCC->PLLCFGR, RCC_PLLCFGR_PLLN) >> RCC_PLLCFGR_PLLN_Pos;
                    frequency =
                        (pllvco * plln) / ((READ_BIT(RCC->PLLCFGR, RCC_PLLCFGR_PLLP) >> RCC_PLLCFGR_PLLP_Pos) + 1U);
                }
            }
        ...
        case RCC_PERIPHCLK_TIM1:
            ...
            if (srcclk == RCC_TIM1CLKSOURCE_PLL) /* PLL ? */
            {
                if (__HAL_RCC_GET_PLLCLKOUT_CONFIG(RCC_PLLQCLK) != 0U)
                {
                    ...
                    plln = READ_BIT(RCC->PLLCFGR, RCC_PLLCFGR_PLLN) >> RCC_PLLCFGR_PLLN_Pos;
                    frequency =
                        (pllvco * plln) / ((READ_BIT(RCC->PLLCFGR, RCC_PLLCFGR_PLLQ) >> RCC_PLLCFGR_PLLQ_Pos) + 1U);
                }
            }
...
to calculate directly reload value with target and synchronization frequencies
values
...
{
    /* NOTE : This function should not be modified, when the callback is needed,
              the @ref HAL_RCCEx_CRS_SyncWarnCallback should be implemented in
       the user file
     */
}
...
{
    /* NOTE : This function should not be modified, when the callback is needed,
              the @ref HAL_RCCEx_CRS_ExpectedSyncCallback should be implemented
       in the user file
     */
}
