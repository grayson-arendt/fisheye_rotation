...
HAL_StatusTypeDef HAL_UART_Init(UART_HandleTypeDef *huart)
{
    ...
    /* For some items, configuration requires to be done prior TE and RE bits
     * are set */
    ...
    /* TEACK and/or REACK to check before moving huart->gState and
     * huart->RxState to Ready */
...
HAL_StatusTypeDef HAL_UART_UnRegisterCallback(UART_HandleTypeDef *huart, HAL_UART_CallbackIDTypeDef CallbackID)
{
    ...
    if (HAL_UART_STATE_READY == huart->gState)
    {
        ...
        case HAL_UART_RX_FIFO_FULL_CB_ID:
            huart->RxFifoFullCallback = HAL_UARTEx_RxFifoFullCallback; /* Legacy weak
                                                                          RxFifoFullCallback     */
            break;

        case HAL_UART_TX_FIFO_EMPTY_CB_ID:
            huart->TxFifoEmptyCallback = HAL_UARTEx_TxFifoEmptyCallback; /* Legacy weak
                                                                            TxFifoEmptyCallback    */
            break;
 ...
 * @note   When UART parity is not enabled (PCE = 0), and Word Length is
 * configured to 9 bits (M1-M0 = 01), address of user data buffer containing
 * data to be sent, should be aligned on a half word frontier (16 bits) (as sent
 * data will be handled using u16 pointer cast). Depending on compilation chain,
 *         use of specific alignment compilation directives or pragmas might be
...
HAL_StatusTypeDef HAL_UART_Transmit(UART_HandleTypeDef *huart, const uint8_t *pData, uint16_t Size, uint32_t Timeout)
{
    ...
    if (huart->gState == HAL_UART_STATE_READY)
    {
        ...
        /* In case of 9bits/No Parity transfer, pData buffer provided as input
           parameter should be aligned on a u16 frontier, as data to be filled
           into TDR will be handled through a u16 cast. */
 ...
 * @note   When UART parity is not enabled (PCE = 0), and Word Length is
 * configured to 9 bits (M1-M0 = 01), address of user data buffer for storing
 * data to be received, should be aligned on a half word frontier (16 bits) (as
 * received data will be handled using u16 pointer cast). Depending on
 * compilation chain, use of specific alignment compilation directives or
 * pragmas might be required to ensure proper alignment for pData.
 * @param huart   UART handle.
 * @param pData   Pointer to data buffer (u8 or u16 data elements).
 ...
 * configured to 9 bits (M1-M0 = 01), address of user data buffer containing
 * data to be sent, should be aligned on a half word frontier (16 bits) (as sent
 * data will be handled by DMA from halfword frontier). Depending on compilation
 * chain, use of specific alignment compilation directives or pragmas might be
 * required to ensure proper alignment for pData.
 * @param huart UART handle.
 * @param pData Pointer to data buffer (u8 or u16 data elements).
 * @param Size  Amount of data elements (u8 or u16) to be sent.
...
HAL_StatusTypeDef HAL_UART_Transmit_DMA(UART_HandleTypeDef *huart, const uint8_t *pData, uint16_t Size)
{
    ...
    if (huart->gState == HAL_UART_STATE_READY)
    {
        ...
        /* In case of 9bits/No Parity transfer, pData buffer provided as input
           parameter should be aligned on a u16 frontier, as data copy into TDR
           will be handled by DMA from a u16 frontier. */
        if ((huart->Init.WordLength == UART_WORDLENGTH_9B) && (huart->Init.Parity == UART_PARITY_NONE))
        {
            if ((((uint32_t)pData) & 1U) != 0U)
            {
        ...
        if (huart->hdmatx != NULL)
        {
            ...
            if (HAL_DMA_Start_IT(huart->hdmatx, (uint32_t)huart->pTxBuffPtr, (uint32_t)&huart->Instance->TDR, Size) !=
                HAL_OK)
            {
 ...
 * configured to 9 bits (M1-M0 = 01), address of user data buffer for storing
 * data to be received, should be aligned on a half word frontier (16 bits) (as
 * received data will be handled by DMA from halfword frontier). Depending on
 * compilation chain, use of specific alignment compilation directives or
 * pragmas might be required to ensure proper alignment for pData.
 * @param huart UART handle.
 * @param pData Pointer to data buffer (u8 or u16 data elements).
...
HAL_StatusTypeDef HAL_UART_Receive_DMA(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size)
{
    ...
    if (huart->RxState == HAL_UART_STATE_READY)
    {
        ...
        /* In case of 9bits/No Parity transfer, pData buffer provided as input
           parameter should be aligned on a u16 frontier, as data copy from RDR
           will be handled by DMA from a u16 frontier. */
        if ((huart->Init.WordLength == UART_WORDLENGTH_9B) && (huart->Init.Parity == UART_PARITY_NONE))
        {
            if ((((uint32_t)pData) & 1U) != 0U)
...
HAL_StatusTypeDef HAL_UART_DMAResume(UART_HandleTypeDef *huart)
{
    ...
    if (huart->RxState == HAL_UART_STATE_BUSY_RX)
    {
        ...
        /* Re-enable PE and ERR (Frame error, noise error, overrun error)
         * interrupts */
...
HAL_StatusTypeDef HAL_UART_DMAStop(UART_HandleTypeDef *huart)
{
    /* The Lock is not implemented on this API to allow the user application
       ...
       the DMA TX/RX Transfer or Half Transfer complete interrupt is generated
       if the DMA transfer interruption occurs at the middle or at the end of
...
HAL_StatusTypeDef HAL_UART_Abort(UART_HandleTypeDef *huart)
{
    ...
    if (HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAT))
    {
        ...
        /* Abort the UART DMA Tx channel : use blocking DMA Abort API (no
         * callback) */
        if (huart->hdmatx != NULL)
        {
    ...
    if (HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAR))
    {
        ...
        /* Abort the UART DMA Rx channel : use blocking DMA Abort API (no
         * callback) */
        if (huart->hdmarx != NULL)
        {
            /* Set the UART DMA Abort callback to Null.
...
HAL_StatusTypeDef HAL_UART_Abort_IT(UART_HandleTypeDef *huart)
{
    ...
    if (HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAT))
    {
        ...
        /* Abort the UART DMA Tx channel : use non blocking DMA Abort API
         * (callback) */
        if (huart->hdmatx != NULL)
        {
            /* UART Tx DMA Abort callback has already been initialised :
               will lead to call HAL_UART_AbortCpltCallback() at end of DMA
               abort procedure */
    ...
    if (HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAR))
    {
        ...
        /* Abort the UART DMA Rx channel : use non blocking DMA Abort API
         * (callback) */
        if (huart->hdmarx != NULL)
        {
...
HAL_StatusTypeDef HAL_UART_AbortTransmit_IT(UART_HandleTypeDef *huart)
{
    ...
    if (HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAT))
    {
        ...
        if (huart->hdmatx != NULL)
        {
            ...
            if (HAL_DMA_Abort_IT(huart->hdmatx) != HAL_OK)
            {
                /* Call Directly huart->hdmatx->XferAbortCallback function in
                 * case of error */
...
HAL_StatusTypeDef HAL_UART_AbortReceive_IT(UART_HandleTypeDef *huart)
{
    ...
    if (HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAR))
    {
        ...
        if (huart->hdmarx != NULL)
        {
            ...
            if (HAL_DMA_Abort_IT(huart->hdmarx) != HAL_OK)
            {
                /* Call Directly huart->hdmarx->XferAbortCallback function in
                 * case of error */
...
void HAL_UART_IRQHandler(UART_HandleTypeDef *huart)
{
            ...
            */
                ...
                ((errorcode & (HAL_UART_ERROR_RTO | HAL_UART_ERROR_ORE)) != 0U))
            {
                /* Blocking error : transfer is aborted
                   Set the UART state ready to be able to start again the
                   process, Disable Rx Interrupts, and disable Rx DMA request,
                   if ongoing */
                UART_EndRxTransfer(huart);
                ...
                if (HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAR))
                {
                    ...
                    {
                        /* Set the UART DMA Abort callback :
                           will lead to call HAL_UART_ErrorCallback() at end of
                           DMA abort procedure */
                        ...
                        if (HAL_DMA_Abort_IT(huart->hdmarx) != HAL_OK)
                        {
                            /* Call Directly huart->hdmarx->XferAbortCallback
        ...
        ((cr1its & USART_ISR_IDLE) != 0U))
    {
        ...
        {
            /* DMA mode enabled */
            /* Check received length : If all expected data are received, do
               nothing, (DMA cplt callback will be called). Otherwise, if at
               least one data has already been received, IDLE event is to be
               notified to user */
            ...
            if ((nb_remaining_rx_data > 0U) && (nb_remaining_rx_data < huart->RxXferSize))
            {
                ...
                {
                    /* Disable PE and ERR (Frame error, noise error, overrun
                     * error) interrupts */
                    ATOMIC_CLEAR_BIT(huart->Instance->CR1, USART_CR1_PEIE);
                    ATOMIC_CLEAR_BIT(huart->Instance->CR3, USART_CR3_EIE);

                    /* Disable the DMA transfer for the receiver request by
                       resetting the DMAR bit in the UART CR3 register */
                    ATOMIC_CLEAR_BIT(huart->Instance->CR3, USART_CR3_DMAR);
                ...
                /* Initialize type of RxEvent that correspond to RxEvent
                   callback execution; In this case, Rx Event type is Idle Event
                 */
            ...
            {
                /* If DMA is in Circular mode, Idle event is to be reported to
                   user even if occurring after a Transfer Complete event from
                   DMA */
                if (nb_remaining_rx_data == huart->RxXferSize)
                {
                    ...
                    {
                        /* Initialize type of RxEvent that correspond to RxEvent
                           callback execution; In this case, Rx Event type is
                           Idle Event */
                        huart->RxEventType = HAL_UART_RXEVENT_IDLE;
        ...
        else
        {
            ...
            /* Check received length : If all expected data are received, do
               nothing. Otherwise, if at least one data has already been
               received, IDLE event is to be notified to user */
            ...
            if ((huart->RxXferCount > 0U) && (nb_rx_data > 0U))
            {
                ...
                /* Disable the UART Error Interrupt:(Frame error, noise error,
                 * overrun error) and RX FIFO Threshold interrupt */
                ATOMIC_CLEAR_BIT(huart->Instance->CR3, (USART_CR3_EIE | USART_CR3_RXFTIE));
    ...
    if (((isrflags & USART_ISR_WUF) != 0U) && ((cr3its & USART_CR3_WUFIE) != 0U))
    {
        ...
        /* UART Rx state is not reset as a reception process might be ongoing.
           If UART handle state fields need to be reset to READY, this could be
           done in Wakeup callback */
...
__weak void HAL_UART_TxHalfCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    /* NOTE: This function should not be modified, when the callback is needed,
             the HAL_UART_TxHalfCpltCallback can be implemented in the user
       file.
     */
}
...
__weak void HAL_UART_RxHalfCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    /* NOTE: This function should not be modified, when the callback is needed,
             the HAL_UART_RxHalfCpltCallback can be implemented in the user
       file.
     */
}
...
__weak void HAL_UART_AbortCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    /* NOTE : This function should not be modified, when the callback is needed,
              the HAL_UART_AbortCpltCallback can be implemented in the user
       file.
     */
}
...
__weak void HAL_UART_AbortReceiveCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    /* NOTE : This function should not be modified, when the callback is needed,
              the HAL_UART_AbortReceiveCpltCallback can be implemented in the
       user file.
     */
}
...
__weak void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
    ...
    /* NOTE : This function should not be modified, when the callback is needed,
              the HAL_UARTEx_RxEventCallback can be implemented in the user
       file.
     */
}
...
HAL_StatusTypeDef HAL_HalfDuplex_EnableReceiver(UART_HandleTypeDef *huart)
{
    ...
    /* Enable the USART's receive interface by setting the RE bit in the USART
     * CR1 register */
...
HAL_StatusTypeDef UART_CheckIdleState(UART_HandleTypeDef *huart)
{
    ...
    if ((huart->Instance->CR1 & USART_CR1_RE) == USART_CR1_RE)
    {
        ...
        {
            /* Disable RXNE, PE and ERR (Frame error, noise error, overrun
        ...
        if (Timeout != HAL_MAX_DELAY)
        {
                ...
                (Flag != UART_FLAG_TC))
            {
                if (__HAL_UART_GET_FLAG(huart, UART_FLAG_ORE) == SET)
                {
                    ...
                    /* Blocking error : transfer is aborted
                    Set the UART state ready to be able to start again the
                    process, Disable Rx Interrupts if ongoing */
                    UART_EndRxTransfer(huart);
...
HAL_StatusTypeDef UART_Start_Receive_DMA(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size)
{
    ...
    if (huart->hdmarx != NULL)
    {
        ...
        if (HAL_DMA_Start_IT(huart->hdmarx, (uint32_t)&huart->Instance->RDR, (uint32_t)huart->pRxBuffPtr, Size) !=
            HAL_OK)
        {
            /* Set error code to DMA */
            huart->ErrorCode = HAL_UART_ERROR_DMA;
...
static void UART_DMATransmitCplt(DMA_HandleTypeDef *hdma)
{
    ...
    {
        /* Disable the DMA transfer for transmit request by resetting the DMAT
