 ...
 * received data will be handled using uint16_t pointer cast). Depending on
                                           ...
                                           uint32_t Timeout)
{
    ...
    if (huart->RxState == HAL_UART_STATE_READY)
    {
        ...
        /* In case of 9bits/No Parity transfer, pData buffer provided as input
           parameter should be aligned on a uint16_t frontier, as data to be
           received from RDR will be handled through a uint16_t cast. */
        ...
        *RxLen = 0U;
        ...
        while (huart->RxXferCount > 0U)
        {
            ...
            if (__HAL_UART_GET_FLAG(huart, UART_FLAG_IDLE))
            {
                ...
                /* If Set, but no data ever received, clear flag without exiting
                 * loop */
                /* If Set, and data has already been received, this means Idle
                 * Event is valid : End reception */
                if (*RxLen > 0U)
                {
    ...
    if (huart->RxState == HAL_UART_STATE_READY)
    {
        ...
        else
        {
            /* In case of errors already pending when reception is started,
               ...
               abortion. (Overrun error for instance). In such case Reception
               Type has been reset to HAL_UART_RECEPTION_STANDARD. */
            status = HAL_ERROR;
        }
...
HAL_StatusTypeDef HAL_UARTEx_ReceiveToIdle_DMA(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size)
{
    ...
    if (huart->RxState == HAL_UART_STATE_READY)
    {
        ...
        /* In case of 9bits/No Parity transfer, pData buffer provided as input
           parameter should be aligned on a uint16_t frontier, as data copy from
           RDR will be handled by DMA from a uint16_t frontier. */
        if ((huart->Init.WordLength == UART_WORDLENGTH_9B) && (huart->Init.Parity == UART_PARITY_NONE))
        {
            if ((((uint32_t)pData) & 1U) != 0U)
            {
 ...
 * of Rx Event Callback, in order to provide the accurate value.
 * @note  In Interrupt Mode:
 *        - HAL_UART_RXEVENT_TC : when Reception has been completed (expected nb
 * of data has been received).
 *        - HAL_UART_RXEVENT_IDLE : when Idle event occurred prior reception has
 * been completed.
 * @note  In DMA Mode:
 *        - HAL_UART_RXEVENT_TC : when Reception has been completed (expected nb
 * of data has been received).
 ...
 *        When DMA is configured in Normal Mode, HT event does not stop
 * Reception process; When DMA is configured in Circular Mode, HT, TC or IDLE
 * events don't stop Reception process;
 * @param  huart UART handle.
