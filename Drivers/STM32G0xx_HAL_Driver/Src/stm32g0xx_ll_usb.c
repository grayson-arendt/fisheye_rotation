...
static HAL_StatusTypeDef USB_HC_BULK_DB_StartXfer(USB_DRD_TypeDef *USBx, USB_DRD_HCTypeDef *hc, uint32_t ch_reg,
                                                  uint32_t *len);

static HAL_StatusTypeDef USB_HC_ISO_DB_StartXfer(USB_DRD_TypeDef *USBx, USB_DRD_HCTypeDef *hc, uint32_t len);
...
void USB_WritePMA(USB_DRD_TypeDef const *USBx, uint8_t *pbUsrBuf, uint16_t wPMABufAddr, uint16_t wNBytes)
{
    ...
    /* Due to the PMA access 32bit only so the last non word data should be
     * processed alone */
...
HAL_StatusTypeDef USB_HostInit(USB_DRD_TypeDef *USBx, USB_DRD_CfgTypeDef cfg)
{
    ...
    USBx->CNTR &= ~(USB_CNTR_CTRM | USB_CNTR_PMAOVRM | USB_CNTR_ERRM | USB_CNTR_WKUPM | USB_CNTR_SUSPM | USB_CNTR_DCON |
                              ...
                              uint8_t speed, uint8_t ep_type, uint16_t mps)
{
    ...
    wChRegVal &= ~(USB_CHEP_DEVADDR | USB_CHEP_ADDR | USB_CHEP_LSEP | USB_CHEP_NAK | USB_CHEP_KIND | USB_CHEP_ERRTX |
                   USB_CHEP_ERRRX | (0xFUL << 27));

    /* Set device address and Endpoint number associated to the channel */
    wChRegVal |= (((uint32_t)dev_address << USB_CHEP_DEVADDR_Pos) | ((uint32_t)epnum & 0x0FU));
 ...
 * @param  db_state double state can be
 * USB_DRD_XXX_DBUFF_ENBALE/USB_DRD_XXX_DBUFF_DISABLE
 * @retval HAL status
 */
...
HAL_StatusTypeDef USB_HC_StartXfer(USB_DRD_TypeDef *USBx, USB_DRD_HCTypeDef *hc)
{
    ...
    if (hc->ch_dir == CH_IN_DIR) /* In Channel */
    {
        ...
        {
            if ((hc->ep_type == EP_TYPE_BULK) || (hc->ep_type == EP_TYPE_INTR))
            {
...
{
    USBx->ISTR &= ~(USB_ISTR_DIR | USB_ISTR_L1REQ | USB_ISTR_ESOF | USB_ISTR_SOF | USB_ISTR_RESET | USB_ISTR_DCON |
