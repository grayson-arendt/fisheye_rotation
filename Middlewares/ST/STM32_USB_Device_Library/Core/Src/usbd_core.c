...
USBD_StatusTypeDef USBD_UnRegisterClassComposite(USBD_HandleTypeDef *pdev)
{
    ...
    {
        /* Check if the class correspond to the requested type and if it is
         * active */
        if (pdev->tclasslist[idx1].Active == 1U)
        {
            ...
            if (pdev->pClass[pdev->classId] != NULL)
            {
                ...
                {
#if (USBD_DEBUG_LEVEL > 1U)
                    USBD_ErrLog("Class DeInit didn't succeed!, can't "
                                "unregister selected class");
#endif /* (USBD_DEBUG_LEVEL > 1U) */
            ...
            /* Free the class location in classes table and reset its parameters
             * to zero */
    ...
    if (epnum == 0U)
    {
        ...
        if (pdev->ep0_state == USBD_EP0_DATA_OUT)
        {
            ...
            else
            {
                ...
                {
                case USB_REQ_RECIPIENT_DEVICE:
                    /* Device requests must be managed by the first
                       instantiated class (or duplicated by all classes for
                       simplicity) */
                    idx = 0U;
                    break;
...
uint8_t USBD_CoreFindIF(USBD_HandleTypeDef *pdev, uint8_t index)
{
    ...
    for (uint32_t i = 0U; i < USBD_MAX_SUPPORTED_CLASS; i++)
    {
        ...
        if ((pdev->tclasslist[i].Active) == 1U)
        {
            ...
            {
                /* Check if requested Interface matches the current class
                 * interface */
...
uint8_t USBD_CoreFindEP(USBD_HandleTypeDef *pdev, uint8_t index)
{
    ...
    for (uint32_t i = 0U; i < USBD_MAX_SUPPORTED_CLASS; i++)
    {
        ...
        if ((pdev->tclasslist[i].Active) == 1U)
        {
            ...
            {
                /* Check if requested endpoint matches the current class
                 * endpoint */
...
uint8_t USBD_CoreGetEPAdd(USBD_HandleTypeDef *pdev, uint8_t ep_dir, uint8_t ep_type, uint8_t ClassId)
{
    ...
    for (idx = 0; idx < pdev->tclasslist[ClassId].NumEps; idx++)
    {
        if (((pdev->tclasslist[ClassId].Eps[idx].add & USBD_EP_IN) == ep_dir) &&
            (pdev->tclasslist[ClassId].Eps[idx].type == ep_type) && (pdev->tclasslist[ClassId].Eps[idx].is_used != 0U))
        {
